...
기존 발표 기획 문구에는 `q90 ≈ 15일` 설명이 들어 있지만, 현재 가공 데이터 기준 재집계 결과는 **q80 = 15, q90 = 24**다.
...
이 불일치는 발표 전에 반드시 정리해야 한다. 가장 안전한 방법은 다음 둘 중 하나다.

1. 원본 전처리 코드로 분위수를 다시 계산해 `q90 = 15`를 재검증한다.
2. 현재 산출물을 기준으로 README와 슬라이드를 수정하고, `15일`은 운영 기준 또는 `q80` 기반 설명으로 정리한다.
